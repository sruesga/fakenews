...
class ConvModel(Model):
    ...
    def build_model(self, model_choice):
        if model_choice == '1dconv':
            self.model = tf.keras.Sequential([
                tf.keras.layers.Embedding(self.vocab_size, self.embed_size, input_length=20),
...
if __name__ == '__main__':
    ...
    model.train()
    model.plot_acc('1D Convolution Accuracy', '1dconv')
    model.plot_loss('1D Convolution Loss', '1dconv')
